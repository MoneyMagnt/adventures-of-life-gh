  ...
  const setupJourneysJournalEffects = () => {
    ...
    stops.forEach((stop, index) => {
      ...
      if (
        ...
        const isJuneNext = stop.dataset.stop === "june";
        const cta = document.createElement("a");
        const message = isJuneNext
          ? `Hello Adventures of Life, I want to secure my slot for ${cleanName} on ${date.textContent.trim()}.`
          : `Hello Adventures of Life, I want to ask about ${cleanName} on ${date.textContent.trim()}.`;

        cta.className = "jy-stop-cta";
        cta.href = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(message.trim())}`;
        cta.target = "_blank";
        cta.rel = "noreferrer";
        cta.textContent = isJuneNext ? "Secure your slot now" : `Ask about ${cleanName}`;
